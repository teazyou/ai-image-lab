...
- `.claude/skills/sharpen-definition/SKILL.md` — `/sharpen-definition` skill (orchestrator): parse file/folder, build the flat per-image cell list, then launch a background dynamic Workflow (`fanout.workflow.js`) that splits the images into ≤`total_sub_agents` (default 3) balanced groups and runs one Sonnet/low **mechanical** worker per group, each processing its group **sequentially** (≤`total_sub_agents` generations at once — RAM-safe); relays results, never views/edits/QAs images. Local-only, auto-invocable
...
- `.claude/skills/sharpen-definition/fanout.workflow.js` — `/sharpen-definition` dynamic-Workflow script: splits the cells into ≤`total_sub_agents` (tweakable constant at the top of the file, default 3) balanced groups and fans out one sub-agent per group on `model: sonnet`/`effort: low`, each reading `agent.md` and processing its group **sequentially**. Cells passed in via Workflow `args`
