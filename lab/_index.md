...
- `.claude/skills/api/fanout.workflow.js` — `/api` dynamic-Workflow script: fans out one sub-agent per (image×model) cell on `model: sonnet`/`effort: high`, each reading `agent.md`; passes each cell's `skipFallback` (other models already covering this image) as a skip line. Cells passed in via Workflow `args`
