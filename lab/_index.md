...
- `.claude/skills/api/SKILL.md` — `/api` skill (orchestrator): pre-flight, build the (image×model) cell list, then launch a background dynamic Workflow (`fanout.workflow.js`) that runs one Sonnet/xhigh worker per cell; chainable via "same params: …"; relays results, never views/edits images. PAID → `disable-model-invocation` (explicit `/api` only)
